...
describe 'codenamephp_php::7.4' do
  context 'When all attributes are default' do
    ...
    it 'installs php7.4 and additional packages' do
      expect(chef_run).to install_codenamephp_php_package('install php 7.4')
    end
  ...
  context 'With additional packages and php package' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['codenamephp_php']['7.4']['package_name'] = 'some package'
        node.normal['codenamephp_php']['7.4']['additional_packages'] = %w(package1 package2 package3)
    ...
    it 'installs php7.1 and additional package using custom values' do
      expect(chef_run).to install_codenamephp_php_package('install php 7.4').with(
