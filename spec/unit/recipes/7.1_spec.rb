...
describe 'codenamephp_php::7.1' do
  context 'When all attributes are default' do
    ...
    it 'installs php7.1 and additional packages' do
      expect(chef_run).to install_codenamephp_php_package('install php 7.1')
    end
    ...
    it 'installs xdebug' do
      expect(chef_run).to install_codenamephp_php_xdebug('install xdebug').with(php_versions: %w[7.1])
    end
  end
  ...
  context 'With additional packages and php package' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['codenamephp_php']['7.1']['package_name'] = 'some package'
    ...
    it 'installs php7.1 and additional package using custom values' do
      expect(chef_run).to install_codenamephp_php_package('install php 7.1').with(
